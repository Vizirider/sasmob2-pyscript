...
def remove_dir(dir_to_remove):
    try:
        ...
        print("Error while removing directory: %s : %s" % (dir_to_remove, e.strerror))

def remove_zip_files():
    files = os.listdir(os.getcwd())
    for file in files:
        if file.startswith("video") and file.endswith(".zip"):
...
def make_dir(dir_name):
    ...
    if not os.path.exists(directory):
        try:
            ...
            print("Error while making directory: %s" % error)
